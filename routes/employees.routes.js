...
router.post('/employees', (req, res) => {
  ...
  req.db.collection('employees').insertOne({ department: department, firstName: firstName, lastName: lastName }, err => {
    if(err) res.status(500).json({ message: err });
...
router.put('/employees/:id', (req, res) => {
  const { department, firstName, lastName } = req.body;
  req.db.collection('employees').updateOne({ _id: ObjectId(req.params.id) }, { $set: { department: department, firstName: firstName, lastName: lastName }}, err => {
    if(err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  });
});
